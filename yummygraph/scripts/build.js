#!/usr/bin/env node
/**
 * Build script that compiles yummygraph and inlines yummygraph-shared into the dist.
 *
 * Steps:
 *  1. Build yummygraph-shared (tsc)
 *  2. Build yummygraph (tsc)
 *  3. Copy yummygraph-shared/dist → dist/_shared
 *  4. Rewrite bare 'yummygraph-shared' specifiers → relative paths
 */
...
const SHARED_ROOT = path.resolve(ROOT, '..', 'yummygraph-shared');
const DIST = path.join(ROOT, 'dist');
const SHARED_DEST = path.join(DIST, '_shared');
const DEFAULT_BUILD_TIMEOUT_MS = 300_000;

function getBuildTimeoutMs() {
  const raw = process.env.YUMMYGRAPH_BUILD_TIMEOUT_MS;
  ...
  console.warn(
    `[build] ignoring invalid YUMMYGRAPH_BUILD_TIMEOUT_MS=${JSON.stringify(raw)}; using ${DEFAULT_BUILD_TIMEOUT_MS}ms`,
  );
...
// monorepo sibling `yummygraph-shared` does not exist and `dist/` is
...
if (!fs.existsSync(SHARED_ROOT)) {
  ...
  console.error(
    `[build] yummygraph-shared not found at ${SHARED_ROOT} and no dist/ exists.\n` +
      'Are you running from the monorepo checkout? Run `npm install` from the repo root first.',
  );
  process.exit(1);
}

// ── 1. Build yummygraph-shared ───────────────────────────────────────
console.log('[build] compiling yummygraph-shared…');
const tscCmd =
...
// ── 2. Build yummygraph ──────────────────────────────────────────────
console.log('[build] compiling yummygraph…');
...
// ── 4. Rewrite imports ─────────────────────────────────────────────
console.log('[build] rewriting yummygraph-shared imports…');
let rewritten = 0;

function rewriteFile(filePath) {
  const content = fs.readFileSync(filePath, 'utf-8');
  if (!content.includes('yummygraph-shared')) return;
  ...
  const updated = content
    .replace(/from\s+['"]yummygraph-shared['"]/g, `from '${relImport}'`)
    .replace(/import\(\s*['"]yummygraph-shared['"]\s*\)/g, `import('${relImport}')`);
...
// ── 6. Build & copy web UI ──────────────────────────────────────────
const WEB_ROOT = path.resolve(ROOT, '..', 'yummygraph-web');
const WEB_DEST = path.join(DIST, '..', 'web');

if (fs.existsSync(path.join(WEB_ROOT, 'package.json'))) {
  console.log('[build] building yummygraph-web…');
  if (!fs.existsSync(path.join(WEB_ROOT, 'node_modules'))) {
    console.log('[build] installing yummygraph-web dependencies…');
  ...
  // Copy dist → yummygraph/web/ (shipped in the npm package)
  fs.rmSync(WEB_DEST, { recursive: true, force: true });
  fs.cpSync(path.join(WEB_ROOT, 'dist'), WEB_DEST, { recursive: true });
  console.log('[build] copied web UI → yummygraph/web/');
} else {
  console.log('[build] skipping web UI (yummygraph-web not found)');
}
