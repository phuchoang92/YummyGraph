 ...
 * Enabled with `YUMMYGRAPH_DEBUG_HEAP=1` or `YUMMYGRAPH_PROFILE_DEFERRED=1`.
 */
...
export const isDebugHeapEnabled = (): boolean =>
  parseTruthyEnv(process.env.YUMMYGRAPH_DEBUG_HEAP) || isDeferredResolutionProfileEnabled();
 ...
 * `YUMMYGRAPH_HEAP_PROBE_FILE=/path` — each probe is then ALSO appended to
...
export const logHeapProbe = (label: string, detail?: string): void => {
  ...
  const line = `[yummygraph-heap] ${label} used_mb=${heapUsedMb()} rss_mb=${rssMb()}${suffix}\n`;
  process.stderr.write(line);
  const file = process.env.YUMMYGRAPH_HEAP_PROBE_FILE;
  if (file) {
    try {
