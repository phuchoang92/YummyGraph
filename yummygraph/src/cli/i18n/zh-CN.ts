...
export const zhCN = {
  ...
  'common.runAnalyze': '请在 git 仓库中运行 `yummygraph analyze` 进行索引。',
  'common.runAnalyzeShort': '运行：yummygraph analyze',
  ...
  'status.rebuildLadybug': '运行：yummygraph analyze   （使用 LadybugDB 重建索引）',
  ...
  'status.stale': '⚠️ 已过期（重新运行 yummygraph analyze）',
  'clean.deleteAll': '将删除 {{count}} 个仓库的 YummyGraph 索引：',
  'clean.deletedRepo': '已删除：{{name}}（{{storagePath}}）',
  'clean.notFoundHere': '当前目录未找到已索引仓库。',
  'clean.deleteCurrent': '将删除该仓库的 YummyGraph 索引：{{repoName}}',
  ...
  'remove.deleteTarget': '将删除该仓库的 YummyGraph 索引：{{name}}',
  'remove.removed': '已移除：{{name}}',
  'remove.failed': '移除 {{name}} 失败：{{message}}',
  'tool.noIndexed': 'YummyGraph：未找到已索引仓库。请运行：yummygraph analyze',
  'tool.usage.query': '用法：yummygraph query <搜索词>',
  'tool.usage.context': '用法：yummygraph context <符号名> [--uid <uid>] [--file <路径>]',
  'tool.usage.impact':
    '用法：yummygraph impact <符号名> [--uid <uid>] [--file <路径>] [--kind <类型>] [--direction upstream|downstream]',
  'tool.usage.cypher': '用法：yummygraph cypher <Cypher 查询>',
  ...
  'serve.walCorruption': '\nYummyGraph 服务器无法启动：索引 WAL 文件已损坏。\n  {{suggestion}}\n',
  'serve.portInUse':
    '\nYummyGraph 服务器启动失败：\n  {{message}}\n\n  端口 {{port}} 已被占用。可选择：\n    1. 停止占用端口 {{port}} 的其他进程\n    2. 使用其他端口：yummygraph serve --port 4748\n',
  'serve.startFailed': '\nYummyGraph 服务器启动失败：\n  {{message}}\n',
  'doctor.title': 'YummyGraph 诊断',
  'doctor.runtime': '运行时',
  ...
  'help.description.root': 'YummyGraph 本地 CLI 和 MCP 服务器',
  ...
  'help.command.index.description': '将现有 .yummygraph/ 文件夹注册到全局注册表（无需重新分析）',
  ...
  'help.command.clean.description': '删除当前仓库的 YummyGraph 索引',
  'help.command.remove.description':
    '删除已注册仓库的 YummyGraph 索引（按别名、名称或绝对路径）。与 `clean` 不同，不要求位于仓库内；未知目标会幂等处理。',
  ...
  'help.command.publish.description':
    '通知 understand-quickly 注册表该仓库已有新的 YummyGraph 索引。需显式启用：要求 UNDERSTAND_QUICKLY_TOKEN（对 looptech-ai/understand-quickly 具备 `Repository dispatches: write` 的细粒度 PAT）。无 token 时不执行。参见 https://github.com/looptech-ai/understand-quickly。',
  ...
  'help.option.analyze.skipAgentsMd': '跳过更新 AGENTS.md 和 CLAUDE.md 中的 yummygraph 区块',
  'help.option.analyze.noStats': '从 AGENTS.md 和 CLAUDE.md 中省略易变的文件/符号计数',
  'help.option.analyze.skipSkills':
    '跳过安装 .claude/skills/yummygraph/ 下的标准 YummyGraph skill 文件。不抑制 --skills 生成的社区 skill（位于 .claude/skills/generated/）。使用 --index-only 可跳过所有 AI 上下文文件注入。',
  ...
  'help.option.analyze.name':
    '在 ~/.yummygraph/registry.json 中使用自定义名称注册该仓库（用于区分路径 basename 相同的仓库，例如两个不同的 .../app 目录）',
  ...
  'help.option.wiki.apiKey': 'LLM API key 或 Azure api-key（保存到 ~/.yummygraph/config.json）',
  ...
  'help.analyze.environment':
    '\n环境变量：\n  YUMMYGRAPH_NO_GITIGNORE=1   跳过 .gitignore 解析（仍读取 .yummygraphignore）\n  YUMMYGRAPH_MAX_FILE_SIZE=N  覆盖大文件跳过阈值（KB）。默认 512，最大 32768。\n  YUMMYGRAPH_WORKER_SUB_BATCH_TIMEOUT_MS=N  Worker 空闲超时（毫秒）。默认 30000。\n  YUMMYGRAPH_WAL_CHECKPOINT_THRESHOLD=N  LadybugDB WAL 自动 checkpoint 阈值（字节，默认 67108864 = 64 MiB；-1 保持 Ladybug 默认约 16 MiB）。\n  YUMMYGRAPH_WORKER_SUB_BATCH_MAX_BYTES=N  Worker 作业字节预算。默认 8388608。\n  YUMMYGRAPH_WORKER_POOL_SIZE=N  解析 worker 数量覆盖值。默认 cores-1，最多 16。\n  YUMMYGRAPH_PARSE_CHUNK_CONCURRENCY=N  并发进行中的解析分块数。默认 2。\n  YUMMYGRAPH_WORKER_MAX_RESPAWNS_PER_SLOT=N  每个 slot 丢弃前允许的最大替换进程数。默认 3。\n  YUMMYGRAPH_WORKER_MAX_CUMULATIVE_TIMEOUT_MS=N  每个作业的总重试墙钟时间。默认 5 倍子批次超时。\n  YUMMYGRAPH_WORKER_CONSECUTIVE_FAILURE_THRESHOLD=N  每个 slot 触发熔断的死亡次数。默认 max(3, poolSize)。\n  YUMMYGRAPH_EMBEDDING_THREADS=N  限制 --embeddings 的本地 ONNX CPU 线程数。\n  YUMMYGRAPH_SEMANTIC_EXACT_SCAN_LIMIT=N  exact-scan 回退的最大嵌入分块数。默认 10000。\n\n当参数和对应环境变量同时提供时，参数优先。\n\n提示：`.yummygraphignore` 支持 `.gitignore` 风格的取反。比如添加\n     `!__tests__/` 可以索引默认自动过滤的目录（#771）。',
