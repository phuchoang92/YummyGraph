...
> protection lives in `yummygraph/test/integration/parse-impl-large-fixture.test.ts`
...
`yummygraph/test/integration/parse-impl-large-fixture.test.ts` (U6):
...
`YUMMYGRAPH_CHUNK_BYTE_BUDGET=64` forces multi-chunk parsing on this small
...
- RAM
- Node version
- yummygraph commit SHA (so the snapshot is anchored to a tree, not "main")
...
```bash
# From the yummygraph/ subdir:
cd yummygraph
# The worker pool is the sole parse path, so every run needs the dist worker
# (`npm run build`) and a pool size pinned via YUMMYGRAPH_WORKER_POOL_SIZE.
...
npm run build && \
  YUMMYGRAPH_WORKER_POOL_SIZE=1 \
  YUMMYGRAPH_VERBOSE=1 \
  npx vitest run test/integration/parse-impl-large-fixture.test.ts --reporter=verbose

# Multi-worker path:
npm run build && \
  YUMMYGRAPH_WORKER_POOL_SIZE=4 \
  YUMMYGRAPH_PARSE_CHUNK_CONCURRENCY=2 \
  YUMMYGRAPH_VERBOSE=1 \
  npx vitest run test/integration/parse-impl-large-fixture.test.ts --reporter=verbose
```
...
`yummygraph/bench/scripts/parse-throughput.ts` would automate this — the
...
The U6 integration test (`yummygraph/test/integration/parse-impl-large-fixture.test.ts`)
...
| ------------------------------------------------------------------------- | ------- | ---------- | --------- | ------ | ----------- |
| Worker pool, `--workers 1` (`YUMMYGRAPH_WORKER_POOL_SIZE=1`), concurrency 1 | _TBD_   | _TBD_      | _TBD_     | _TBD_  | 0           |
| Worker pool, `--workers 4`, concurrency 2                                 | _TBD_   | _TBD_      | _TBD_     | _TBD_  | 0           |

**Hardware:** _TBD — record OS, CPU, RAM, Node version, yummygraph SHA at
...
- **CPU-bound, big repo, lots of cores:** raise `YUMMYGRAPH_WORKER_POOL_SIZE`
...
- **Slow files (large minified JS, deep TS types):** raise
  `YUMMYGRAPH_WORKER_SUB_BATCH_TIMEOUT_MS` past 30 000 ms. The cumulative
...
- **Constrained container (cgroup CPU limit):** the pool now uses
  ...
  manual `YUMMYGRAPH_WORKER_POOL_SIZE` override needed unless the auto-
...
- **Long-running host (eval-server, MCP daemon) running back-to-back
  ...
  state leaking across calls. `YUMMYGRAPH_VERBOSE` is similarly snapshot/
  restore-bracketed.
...
- Integration test (mini-benchmark with wall-clock guard): `yummygraph/test/integration/parse-impl-large-fixture.test.ts` (U6)
- Operator env-var reference: `README.md` → Environment variables
- Resilience layer tests: `yummygraph/test/unit/worker-pool-resilience.test.ts`,
