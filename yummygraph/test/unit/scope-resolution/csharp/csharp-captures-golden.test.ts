/**
 * Golden capture-parity test for `emitCsharpScopeCaptures` (PR #1918 follow-up).
 *
 * Pins the exact capture output of `emitCsharpScopeCaptures` across the whole
 * `test/fixtures/lang-resolution/csharp-*` corpus plus a synthetic generated-DAO
 * source, so any future drift in the C# scope-capture path fails CI rather than
 * only being caught by a coarse perf tripwire, the bench fingerprint CI job, or
 * pipeline-level resolver tests.
 *
 * This is a FORWARD-DRIFT guard: it locks in the current (post-#1918, verified)
 * linear output as the baseline. It does not independently re-prove the original
 * pre-optimization parity — that was established during PR #1918.
 *
 ...
 *     because emitCsharpScopeCaptures output is deterministic, and it makes the
...
const GOLDEN_DIR = path.resolve(__dirname, '..', '..', '..', 'fixtures', 'csharp-captures-golden');
...
function snapshotOf(src: string, filePath: string): FixtureSnapshot {
  const matches = emitCsharpScopeCaptures(src, filePath);
  return { captureGroups: matches.length, digest: digestCaptures(matches) };
}

/** All `.cs` files under `lang-resolution/csharp-*`, as sorted repo-relative-ish keys. */
function collectCsharpFixtures(): { key: string; absPath: string }[] {
  const out: { key: string; absPath: string }[] = [];
  for (const entry of fs.readdirSync(FIXTURE_ROOT, { withFileTypes: true })) {
    if (!entry.isDirectory() || !entry.name.startsWith('csharp-')) continue;
...
/**
 * Small deterministic generated-DAO source — the #1918 shape at correctness
 * scale. Reuses the synthetic DAO unit shape from bench/scope-capture/measure.mjs:
 * a namespace with N classes, each carrying fields plus getter/setter methods.
 */
...
function buildSnapshot(): Snapshot {
  const snap: Snapshot = {};
  for (const { key, absPath } of collectCsharpFixtures()) {
    snap[key] = snapshotOf(fs.readFileSync(absPath, 'utf8'), absPath);
  }
  snap['synthetic:dao-20'] = snapshotOf(generateDao(20), 'zz_generated_dao.cs');
...
describe('C# scope captures — golden parity', () => {
  it('matches the committed golden snapshot across all csharp-* fixtures + DAO shape', () => {
    ...
    if (action === 'fail') {
      throw new Error(
        `[csharp-captures-golden] golden file missing at ${GOLDEN_FILE} in CI. A missing golden must ` +
    ...
    if (action === 'regenerate') {
      ...
      console.log(
        `[csharp-captures-golden] ${UPDATE ? 'Regenerated' : 'Created'} golden at ${GOLDEN_FILE}`,
      );
      return;
    }
    ...
    expect(
      snapshot,
      'emitCsharpScopeCaptures output drifted from the committed golden. If this drift is intentional ' +
        '(or the digest scheme changed), regenerate with ' +
        'UPDATE_GOLDEN=1 npx vitest run test/unit/scope-resolution/csharp/csharp-captures-golden.test.ts',
    ).toEqual(expected);
  });
  ...
  it('produces a deterministic digest across repeated runs', () => {
    ...
    expect(digestCaptures(emitCsharpScopeCaptures(src, 'a.cs'))).toBe(
      digestCaptures(emitCsharpScopeCaptures(src, 'a.cs')),
    );
  });

  it('digest is sensitive to capture-match emission order', () => {
    const matches = emitCsharpScopeCaptures(generateDao(6), 'a.cs');
  ...
  it('records a capture-group count for every fixture and the DAO shape', () => {
    const snapshot = buildSnapshot();
    const fixtureKeys = collectCsharpFixtures().map((f) => f.key);
