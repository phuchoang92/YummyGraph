/**
 * Golden capture-parity test for `emitPhpScopeCaptures` (PR #1918 follow-up).
 *
 * Pins the exact capture output of `emitPhpScopeCaptures` across the whole
 * `test/fixtures/lang-resolution/php-*` corpus plus a synthetic generated-DAO
 * source, so any future drift in the PHP scope-capture path fails CI rather than
 ...
 *     because emitPhpScopeCaptures output is deterministic, and it makes the
...
const GOLDEN_DIR = path.resolve(__dirname, '..', '..', '..', 'fixtures', 'php-captures-golden');
...
function snapshotOf(src: string, filePath: string): FixtureSnapshot {
  const matches = emitPhpScopeCaptures(src, filePath);
  return { captureGroups: matches.length, digest: digestCaptures(matches) };
}

/** All `.php` files under `lang-resolution/php-*`, as sorted repo-relative-ish keys. */
function collectPhpFixtures(): { key: string; absPath: string }[] {
  const out: { key: string; absPath: string }[] = [];
  for (const entry of fs.readdirSync(FIXTURE_ROOT, { withFileTypes: true })) {
    if (!entry.isDirectory() || !entry.name.startsWith('php-')) continue;
...
function buildSnapshot(): Snapshot {
  const snap: Snapshot = {};
  for (const { key, absPath } of collectPhpFixtures()) {
    snap[key] = snapshotOf(fs.readFileSync(absPath, 'utf8'), absPath);
  }
  snap['synthetic:dao-20'] = snapshotOf(generateDao(20), 'zz_generated_dao.php');
...
describe('PHP scope captures — golden parity', () => {
  it('matches the committed golden snapshot across all php-* fixtures + DAO shape', () => {
    ...
    if (action === 'fail') {
      throw new Error(
        `[php-captures-golden] golden file missing at ${GOLDEN_FILE} in CI. A missing golden must ` +
    ...
    if (action === 'regenerate') {
      ...
      console.log(
        `[php-captures-golden] ${UPDATE ? 'Regenerated' : 'Created'} golden at ${GOLDEN_FILE}`,
      );
      return;
    }
    ...
    expect(
      snapshot,
      'emitPhpScopeCaptures output drifted from the committed golden. If this drift is intentional ' +
        '(or the digest scheme changed), regenerate with ' +
        'UPDATE_GOLDEN=1 npx vitest run test/unit/scope-resolution/php/php-captures-golden.test.ts',
    ).toEqual(expected);
  });
  ...
  it('produces a deterministic digest across repeated runs', () => {
    ...
    expect(digestCaptures(emitPhpScopeCaptures(src, 'a.php'))).toBe(
      digestCaptures(emitPhpScopeCaptures(src, 'a.php')),
    );
  });

  it('digest is sensitive to capture-match emission order', () => {
    const matches = emitPhpScopeCaptures(generateDao(6), 'a.php');
  ...
  it('records a capture-group count for every fixture and the DAO shape', () => {
    const snapshot = buildSnapshot();
    const fixtureKeys = collectPhpFixtures().map((f) => f.key);
