#!/usr/bin/env node
/**
 * YummyGraph Claude Code Plugin Hook
 *
 ...
 * On Windows, invoke yummygraph.cmd directly (no shell needed).
 */
function runYummyGraphCli(args, cwd, timeout) {
  const isWin = process.platform === 'win32';
  const hookCli = process.env.YUMMYGRAPH_HOOK_CLI_PATH;
  ...
  // Detect whether 'yummygraph' is on PATH (cheap check, no execution)
  let useDirectBinary = false;
  try {
    const which = spawnSync(isWin ? 'where' : 'which', ['yummygraph'], {
      encoding: 'utf-8',
      timeout: 3000,
      stdio: ['pipe', 'pipe', 'pipe'],
      windowsHide: true,
  ...
  if (useDirectBinary) {
    return spawnSync(isWin ? 'yummygraph.cmd' : 'yummygraph', args, {
      encoding: 'utf-8',
      timeout,
      cwd,
      stdio: ['pipe', 'pipe', 'pipe'],
...
function handlePreToolUse(input) {
  ...
  let result = '';
  try {
    const child = runYummyGraphCli(['augment', '--', pattern], cwd, 7000);
