...
2. **Never rename with find-and-replace** in YummyGraph-indexed projects — use `rename` MCP tool with `dry_run: true` first, review `graph` vs `text_search` edits. No separate `yummygraph rename` CLI exists.
...
5. **Preserve embeddings** — plain `npx yummygraph analyze` now preserves any embeddings recorded in `.yummygraph/meta.json` (the previous behavior wiped them). Use `--embeddings` to also generate vectors for new/changed nodes; use `--drop-embeddings` only when an explicit wipe is intended (e.g., model swap).
...
- **Do:** `npx yummygraph analyze --force` to rebuild from scratch. The dirty-flag check forces this automatically when a previous incremental run didn't complete cleanly, but `--force` is the manual escape hatch. Safe to delete the `.yummygraph/parse-cache/` directory (and any legacy `.yummygraph/parse-cache.json`) at any time — content-addressed, will be regenerated.
...
- **Do:** Re-run `npx yummygraph analyze --embeddings` to regenerate. Check the analyze log for a `Warning: could not load cached embeddings` line — if present, the cache restore failed (corrupt DB / schema mismatch) and the rebuild had nothing to preserve. If you intentionally passed `--drop-embeddings`, this is expected.
...
- **Do:** `npx yummygraph analyze` in the target repo; verify `npx yummygraph list` shows it.
- **Why:** MCP discovers repos via `~/.yummygraph/registry.json`, populated by analyze.
...
- **Trigger:** Errors opening `.yummygraph/lbug` while MCP and analyze both run.
