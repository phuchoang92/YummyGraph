...
| Boundary | Rule |
|----------|------|
| **Reads** | `yummygraph/`, `yummygraph-web/`, `eval/`, plugin packages, `.github/`, `.yummygraph/`, docs. |
| **Writes** | Only paths required for the change; keep diffs minimal. Update lockfiles when deps change. |
| **Executes** | `npm`, `npx`, `node` under `yummygraph/` and `yummygraph-web/`; `uv run` for Python under `eval/`; documented CI/dev workflows. |
...
- **Notes:** The YummyGraph CLI indexer does not call an LLM.
...
3. Which **validation commands** you will run (`cd yummygraph && npm test`, `npx tsc --noEmit`).

On long threads, *"Remember: apply all AGENTS.md rules"* re-weights these instructions against context dilution.

## Claude Code hooks

**PreToolUse** hooks can block tools (e.g. `git_commit`) until checks pass. Adapt to this repo: `cd yummygraph && npm test` before commit.
...
| 2026-04-19 | 1.5.0 | Cross-repo impact (#794): `impact`/`query`/`context` accept `repo: "@<group>"` + `service`. Removed `group_query`/`group_contracts`/`group_status` MCP tools; added `yummygraph://group/{name}/contracts` and `yummygraph://group/{name}/status` resources. |
| 2026-04-16 | 1.4.0 | Fixed: web UI description, pre-commit behavior, MCP tools (7->16), added yummygraph-shared, removed stale vite-plugin-wasm gotcha. |
| 2026-04-13 | 1.3.0 | Updated YummyGraph index stats after DAG refactor. |
| 2026-03-24 | 1.2.0 | Fixed yummygraph:start block duplication. |
...
|---------|------|---------|
| **CLI/Core** | `yummygraph/` | TypeScript CLI, indexing pipeline, MCP server. Published to npm. |
| **Web UI** | `yummygraph-web/` | React/Vite thin client. All queries via `yummygraph serve` HTTP API. |
| **Shared** | `yummygraph-shared/` | Shared TypeScript types and constants. |
| Claude Plugin | `yummygraph-claude-plugin/` | Static config for Claude marketplace. |
| Cursor Integration | `yummygraph-cursor-integration/` | Static config for Cursor editor. |
| Eval | `eval/` | Python evaluation harness (Docker + LLM API keys). |

### Running services

```bash
cd yummygraph && npm run dev                 # CLI: tsx watch mode
cd yummygraph-web && npm run dev             # Web UI: Vite on port 5173
npx yummygraph serve                         # HTTP API on port 4747 (from any indexed repo)
```
...
**Web UI (`yummygraph-web/`)**
- `npm test` — vitest (~200 tests)
- `npm run test:e2e` — Playwright (7 spec files; requires `yummygraph serve` + `npm run dev`)
...
- `npm install` in `yummygraph/` triggers `prepare` (builds via `tsc`) and `postinstall` (patches tree-sitter-swift, builds tree-sitter-proto). Native bindings need `python3`, `make`, `g++`.
