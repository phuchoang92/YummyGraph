#!/usr/bin/env python3
"""
Results Analyzer for YummyGraph SWE-bench Evaluation
...
- YummyGraph tool usage patterns
...
def compute_metrics(run_data: dict) -> dict:
    ...
    for instance_id, traj in trajectories.items():
        ...
        gn = info.get("yummygraph", {}).get("metrics", {})
        if gn:
...
@app.command()
def yummygraph_usage(
    results_dir: str = typer.Argument(..., help="Path to results directory"),
):
    """Analyze YummyGraph tool usage patterns across all runs."""
    results_path = Path(results_dir)
    runs = load_run_results(results_path)

    console.print("\n[bold]YummyGraph Tool Usage Analysis[/bold]\n")
    ...
    for run_id, run_data in sorted(runs.items()):
        ...
        for traj in run_data.get("trajectories", {}).values():
            gn = traj.get("info", {}).get("yummygraph", {}).get("metrics", {})
